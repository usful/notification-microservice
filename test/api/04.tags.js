...
describe('Tag', () => {
  ...
  it('should add user to tag successfully', async () => {
    const postRes = await request(server)
      .post('/api/tag/user-0/new-tag-01')
      .send()
      .set('Accept', 'application/json')
      .expect(200);

    expect(postRes.body.data).to.deep.equal({ id: 'user-0', tag: 'new-tag-01' });
  ...
  it('should remove user from tag successfully', async () => {
    const deleteRes = await request(server)
      .delete('/api/tag/user-0/new-tag-01')
      .send()
      .set('Accept', 'application/json')
      .expect(200);

    expect(deleteRes.body.data).to.deep.equal({ id: 'user-0', tag: 'new-tag-01' });
